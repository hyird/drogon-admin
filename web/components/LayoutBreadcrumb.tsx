...
export default function LayoutBreadcrumb() {
  ...
  const pathMap = useMemo(() => {
    ...
    const traverse = (items: Menu.TreeItem[], parents: Menu.TreeItem[] = []) => {
      for (const item of items) {
        if (item.fullPath) {
          map.set(item.fullPath, [...parents, item]);
        }
        if (item.children) {
  ...
  const renderBreadcrumbItem = (data: BreadcrumbItemData) => {
    ...
    const pageChildren =
      item.children?.filter((child) => child.type === "page" && child.fullPath) || [];
    ...
    if (pageChildren.length > 0) {
      const menuItems: MenuProps["items"] = pageChildren.map((child) => ({
        ...
        label: child.name,
        disabled: location.pathname === child.fullPath,
        onClick: () => {
          if (child.fullPath && location.pathname !== child.fullPath) {
            navigate(child.fullPath);
          }
        },
      }));
    ...
    if (item.fullPath && item.type === "page") {
      return (
        <span
