import { Layout, Menu as AntdMenu, Dropdown, Spin, Button, Space, Drawer, Grid } from "antd";
...
export default function AdminLayout() {
  const { token, user, clearAuth, refreshUser } = useAuthStore();
      ...
      return items
        ...
        .map((m) => {
          ...
          if (children && children.length > 0) {
            return {
              key: m.fullPath || m.path || `menu-${m.id}`,
  ...
  const siderContent = (
    <>
      <div className="h-12 m-2 rounded flex items-center justify-center shrink-0 text-white font-medium">
        {!isInitialLoading && <span>{collapsed && !isMobile ? "DA" : "Drogon Admin"}</span>}
      </div>
      {!isInitialLoading && (
