  ...
  const computeFullPath = (nodes: Menu.TreeItem[], parentPath = "") => {
    for (const node of nodes) {
      if (node.path) {
        ...
        node.fullPath = node.path.startsWith("/") ? node.path : `${parentPath}/${node.path}`;
      } else {
        node.fullPath = parentPath;
      }
      if (node.children) {
        computeFullPath(node.children, node.fullPath);
      }
    }
  };
...
export function getPathSegment(
  ...
  const parent = menuMap instanceof Map ? menuMap.get(record.parentId) : menuMap[record.parentId];
