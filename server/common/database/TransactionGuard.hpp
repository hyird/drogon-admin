 ...
 * 使用示例：
 * @code
 * TRANSACTION_DO(dbService_, [this, userId, data](auto& tx) -> Task<void> {
 ...
 * @endcode
 */
#define TRANSACTION_DO(dbService, block) \
    do { \
        auto __tx_guard = co_await TransactionGuard::create(dbService); \
        co_await block(__tx_guard); \
        co_await __tx_guard.commit(); \
    } while(0)
